...
class Tag extends BaseController
{
    ...
    {
        $query = $this->db->query('SELECT tags.id,tags.name  
                                    FROM tags ');

        $data['tags'] = $query->getResult();
    ...
    public function create()
    {
        if ($this->request->getMethod() === 'post' && $this->validate($this->rules)) {
            $query = $this->db->prepare(function ($db) {
                $sql = "INSERT INTO tags( tags.name) 
                    VALUES (?)";
            ...
            session()->setFlashdata('success', "Tag added");
    ...
    {
        $data['tag'] = $this->db->query('SELECT * FROM tags WHERE id=' . $id)->getRowObject();
    ...
    public function update(int $id)
    {
        if ($this->request->getMethod() === 'post' && $this->validate($this->rules)) {
            $query = $this->db->prepare(function ($db) {
                $sql = "UPDATE tags SET tags.name=? WHERE id=?";
            ...
            session()->setFlashdata('success', "Tag edited");
            return redirect()->to(base_url('/tags'));
        }
        $data['tag'] = $this->db->query('SELECT * FROM tags WHERE id=' . $id)->getRowObject();
        $data['errors'] = $this->validator;
    ...
    public function delete(int $id)
    {
        ...
        session()->setFlashdata('success', "Tag deleted");
